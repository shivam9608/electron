...
void HidChooserContext::GrantDevicePermission(
  ...
  if (CanStorePersistentEntry(device)) {
    browser_context_->GrantObjectPermission(origin, DeviceInfoToValue(device),
                                            kHidGrantedDevicesPref);
  } else {
...
bool HidChooserContext::HasDevicePermission(
  ...
  std::vector<std::unique_ptr<base::Value>> object_list =
      browser_context_->GetGrantedObjects(origin, kHidGrantedDevicesPref);
  for (const auto& device_value : object_list) {
    ...
    const auto* serial_number =
        device_value->FindStringKey(kHidSerialNumberKey);
